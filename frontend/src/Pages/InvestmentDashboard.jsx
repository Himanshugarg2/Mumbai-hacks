...
useEffect(() => {
  const fetchAllInvestments = async () => {
    ...
    try {
      const urls = [
        "http://localhost:8000/fds",
        "http://localhost:8000/bonds",
        "http://localhost:8000/savings",
        "http://localhost:8000/mutual-funds?limit=20"
      ];

      const [fd, bonds, savings, mf] = await Promise.all(urls.map(u => fetch(u).then(r => r.json())));

      setInvestmentData({
        fds: fd?.fds || [],
        bonds: bonds?.bonds || [],
        savings: savings?.schemes || [],
        mutualFunds: mf?.results || []
      });
